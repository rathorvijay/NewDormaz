            ...
            <Card>
              <CardContent>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={6}><TextField fullWidth label="Thickness (e.g. 6 inch)" name="thickness" value={form.thickness} onChange={handleChange} required /></Grid>
                  ...
                  <Grid item xs={3}><TextField fullWidth label="Discount (%)" name="discount" type="number" value={form.discount} onChange={handleChange} inputProps={{ min: 0, max: 100 }} /></Grid>
                  ...
                  <Grid item xs={6}><TextField fullWidth label="Return Window (days)" name="returnWindowDays" type="number" value={form.returnWindowDays} onChange={handleChange} inputProps={{ min: 1, max: 30 }} disabled={!form.isReturnable} /></Grid>
                  <Grid item xs={6}><TextField fullWidth label="Tags (comma separated)" name="tags" value={form.tags} onChange={handleChange} placeholder="comfort,ortho,luxury" /></Grid>
                  <Grid item xs={12}><TextField fullWidth label="Features (one per line)" name="features" value={form.features} onChange={handleChange} multiline rows={3} placeholder="Premium foam&#10;5 year warranty&#10;Easy to clean" /></Grid>
                  ...
                  <Grid item xs={12}>
                    <FormControlLabel control={<Switch name="isReturnable" checked={form.isReturnable} onChange={handleChange} color="primary" />} label="Product is returnable" />
                  </Grid>
                </Grid>
              </CardContent>
            </Card>
              ...
              <CardContent>
                ...
                <Grid container spacing={1}>
                  {previews.map((preview, i) => (
                    <Grid item xs={6} key={preview}>
                      <Box sx={{ position: 'relative', borderRadius: 2, overflow: 'hidden', height: 100 }}>
                        <img src={preview} alt="preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        <IconButton size="small" sx={{ position: 'absolute', top: 2, right: 2, bgcolor: 'error.main', color: 'white', '&:hover': { bgcolor: 'error.dark' } }} onClick={() => {
                ...
                {previews.length === 0 && <Box sx={{ textAlign: 'center', py: 4, border: '2px dashed #e0e0e0', borderRadius: 2 }}><Typography color="text.secondary">Upload up to 5 images</Typography></Box>}
              </CardContent>
            </Card>
