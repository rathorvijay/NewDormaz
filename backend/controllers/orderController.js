...
const { attachReturnMetadataToOrders } = require('../utils/returnUtils');
...
const createOrder = asyncHandler(async (req, res) => {
  ...
  try {
    const productRows = preparedProducts
      .map(
        (p) =>
  ...
  const [orderWithMeta] = await attachReturnMetadataToOrders([order]);
  res.status(201).json({ success: true, order: orderWithMeta });
});

const getMyOrders = asyncHandler(async (req, res) => {
  const orders = await Order.find({ userId: req.user._id }).sort({ createdAt: -1 });
  const enrichedOrders = await attachReturnMetadataToOrders(orders);
...
const getOrderById = asyncHandler(async (req, res) => {
  ...
  const [enrichedOrder] = await attachReturnMetadataToOrders([order]);
...
const getAllOrders = asyncHandler(async (req, res) => {
  ...
  const enrichedOrders = await attachReturnMetadataToOrders(orders);
  res.json({ success: true, orders: enrichedOrders, total, page, pages: Math.ceil(total / limit) });
});
...
const cancelOrder = asyncHandler(async (req, res) => {
  ...
  const [enrichedOrder] = await attachReturnMetadataToOrders([order]);
  res.json({ success: true, message: 'Order cancelled successfully', order: enrichedOrder });
});
